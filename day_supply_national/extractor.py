...
class PrescriptionDataExtractor:
    ...
    def _load_nasal_inhalers(self) -> pd.DataFrame:
        ...
        try:
            ...
            data_path = pkg_resources.resource_filename('day_supply_national', 'data/nasal_inhalers.csv')
    ...
    def _load_oral_inhalers(self) -> pd.DataFrame:
        ...
        try:
            import pkg_resources
            data_path = pkg_resources.resource_filename('day_supply_national', 'data/oral_inhaler_products.csv')
            return pd.read_csv(data_path)
        except Exception as e:
    ...
    def _load_insulin_products(self) -> pd.DataFrame:
        ...
        try:
            import pkg_resources
            data_path = pkg_resources.resource_filename('day_supply_national', 'data/insulin_products.csv')
            return pd.read_csv(data_path)
        except Exception as e:
    ...
    def _load_biologic_injectables(self) -> pd.DataFrame:
        ...
        try:
            import pkg_resources
            data_path = pkg_resources.resource_filename('day_supply_national', 'data/biologic_injectables.csv')
            return pd.read_csv(data_path)
        except Exception as e:
    ...
    def _load_nonbiologic_injectables(self) -> pd.DataFrame:
        ...
        try:
            import pkg_resources
            data_path = pkg_resources.resource_filename('day_supply_national', 'data/nonbiologic_injectables.csv')
            return pd.read_csv(data_path)
        except Exception as e:
    ...
    def _load_eyedrop_guidelines(self) -> pd.DataFrame:
        ...
        try:
            import pkg_resources
            data_path = pkg_resources.resource_filename('day_supply_national', 'data/pbm_eyedrop_guidelines.csv')
            return pd.read_csv(data_path)
        except Exception as e:
    ...
    def _load_eyedrop_beyond_use(self) -> pd.DataFrame:
        ...
        try:
            import pkg_resources
            data_path = pkg_resources.resource_filename('day_supply_national', 'data/eyedrop_beyond_use_dates.csv')
            return pd.read_csv(data_path)
        except Exception as e:
    ...
    def _load_ftu_dosing(self) -> pd.DataFrame:
        ...
        try:
            import pkg_resources
            data_path = pkg_resources.resource_filename('day_supply_national', 'data/ftu_dosing_guide.csv')
            return pd.read_csv(data_path)
        except Exception as e:
    ...
    def _load_diabetic_injectables(self) -> pd.DataFrame:
        ...
        try:
            import pkg_resources
            data_path = pkg_resources.resource_filename('day_supply_national', 'data/injectable_diabetic_meds.csv')
            return pd.read_csv(data_path)
        except Exception as e:
    ...
    def _load_insulin_pen_increments(self) -> pd.DataFrame:
        ...
        try:
            import pkg_resources
            data_path = pkg_resources.resource_filename('day_supply_national', 'data/insulin_pen_dosing_increments.csv')
            return pd.read_csv(data_path)
        except Exception as e:
    ...
    def _fuzzy_match_drug_name(self, input_name: str, threshold: float = 0.6) -> Tuple[Optional[str], float]:
        ...
        for drug_name in self.drug_database.keys():
            ...
            if input_name_clean in drug_name or drug_name in input_name_clean:
                score = max(len(input_name_clean) / len(drug_name), 
    ...
    def _calculate_frequency_per_day(self, sig: str) -> float:
        ...
        if any(term in sig_lower for term in ['weekly', 'once a week', 'once weekly', 'every week']):
            return 1.0/7.0  # Once per week = 1/7 per day
        elif any(term in sig_lower for term in ['every other week', 'biweekly', 'every 2 weeks']):
            return 1.0/14.0  # Every other week = 1/14 per day
        elif any(term in sig_lower for term in ['monthly', 'once a month', 'every month']):
            return 1.0/30.0  # Monthly = 1/30 per day
        
        # Common daily frequency patterns
        elif any(term in sig_lower for term in ['once', 'daily', 'qd', 'q.d', 'sid']) and 'twice' not in sig_lower:
            return 1.0
        elif any(term in sig_lower for term in ['twice', 'bid', 'b.i.d']):
            return 2.0
        elif any(term in sig_lower for term in ['thrice', 'tid', 't.i.d', 'three times']):
            return 3.0
        ...
        times_match = re.search(r'(\d+(?:\.\d+)?)\s*times?\s*(?:per\s*)?(?:day|daily)', sig_lower)
        if times_match:
    ...
    def _process_nasal_inhaler(self, drug_data: Dict, quantity: float, sig: str) -> Tuple[float, int, str]:
        ...
        day_supply = int(total_sprays / (sprays_per_dose * frequency)) if frequency > 0 else 30
        
        # Ensure reasonable bounds (7-365 days)
        day_supply = max(7, min(day_supply, 365))
        
        # Standardize sig
        standardized_sig = f"Use {sprays_per_dose} spray(s) {self._frequency_to_text(frequency)}"
        
        return corrected_quantity, day_supply, standardized_sig
    
    def _process_oral_inhaler(self, drug_data: Dict, quantity: float, sig: str) -> Tuple[float, int, str]:
        ...
        calculated_days = int(total_puffs / (puffs_per_dose * frequency)) if frequency > 0 else 30
        ...
        standardized_sig = f"Inhale {puffs_per_dose} puff(s) {self._frequency_to_text(frequency)}"
        
        return corrected_quantity, day_supply, standardized_sig
    
    def _process_insulin(self, drug_data: Dict, quantity: float, sig: str) -> Tuple[float, int, str]:
        ...
        calculated_days = int(total_available_units / (units_per_dose * frequency)) if frequency > 0 else 30
        ...
        standardized_sig = f"Inject {units_per_dose} units {self._frequency_to_text(frequency)}"
        
        return corrected_quantity, day_supply, standardized_sig
    
    def _process_eyedrop(self, drug_name: str, quantity: float, sig: str) -> Tuple[float, int, str]:
        """Process eyedrop prescription - no warnings"""
        # Get PBM guidelines (default to PAAS National)
        if not self.eyedrop_guidelines.empty:
            pbm_data = self.eyedrop_guidelines[self.eyedrop_guidelines['PBM'] == 'PAAS National Default']
            if pbm_data.empty:
        ...
        if not self.eyedrop_beyond_use.empty:
            beyond_use_row = self.eyedrop_beyond_use[
                self.eyedrop_beyond_use['Product_Name'].str.lower().str.contains(
                    drug_name.lower().split()[0], na=False
                )
            ]
        ...
        standardized_sig = f"Instill {drops_per_dose} drop(s) {self._frequency_to_text(frequency)}"
        
        return corrected_quantity, day_supply, standardized_sig
    
    def _process_injectable(self, drug_data: Dict, quantity: float, sig: str, is_biologic: bool) -> Tuple[float, int, str]:
    ...
    def _process_topical_ftu(self, quantity: float, sig: str) -> Tuple[float, int, str]:
        ...
        if isinstance(quantity, str):
            ...
            gram_match = re.search(r'(\d+(?:\.\d+)?)\s*(?:g|gm|gram)', str(quantity).lower())
            if gram_match:
                quantity_grams = float(gram_match.group(1))
            else:
                quantity_grams = float(quantity) if str(quantity).replace('.', '').isdigit() else 30.0
        else:
            quantity_grams = float(quantity)
        
        day_supply = int(quantity_grams / total_grams_per_day) if total_grams_per_day > 0 else 30
    ...
    def extract_prescription_data(self, prescription: PrescriptionInput) -> ExtractedData:
        ...
        if matched_name is None:
            ...
            if any(term in drug_lower for term in ['spray', 'nasal', 'flonase', 'nasacort']):
                medication_type = MedicationType.NASAL_INHALER
            elif any(term in drug_lower for term in ['inhaler', 'hfa', 'diskus', 'albuterol', 'ventolin']) or drug_lower == 'inhaler':
                medication_type = MedicationType.ORAL_INHALER
            elif (any(term in drug_lower for term in ['insulin', 'humalog', 'lantus', 'novolog', 'glargine', 'lispro', 'aspart']) or 
                  drug_lower == 'insulin'):
                medication_type = MedicationType.INSULIN
            elif (any(term in drug_lower for term in ['drops', 'ophth', 'eye', 'timolol', 'latanoprost', 'brimonidine', 'azasite', 'rocklatan', 'rhopressa', 'vyzulta', 'xalatan', 'dorzolamide', 'prednisolone', 'ofloxacin', 'cyclosporine']) or
                  any(term in sig_lower for term in ['drop', 'drops', 'eye', 'eyes', 'instill'])):
                medication_type = MedicationType.EYEDROP
            elif (any(term in drug_lower for term in ['cream', 'ointment', 'gel', 'lotion', 'hydrocortisone', 'betamethasone', 'triamcinolone', 'clobetasol', 'tacrolimus', 'mupirocin']) or
                  any(term in sig_lower for term in ['apply', 'topically', 'affected area', 'skin'])):
        ...
        try:
            ...
            if medication_type == MedicationType.NASAL_INHALER:
                ...
                frequency = self._calculate_frequency_per_day(prescription.sig_directions)
                
                # Generic calculation with bounds
                calculated_days = int(corrected_qty / max(frequency, 1.0)) if corrected_qty > 0 else 30
                day_supply = max(7, min(calculated_days, 365))
            ...
            corrected_qty = float(prescription.quantity) if str(prescription.quantity).replace('.', '').isdigit() else 1.0
            day_supply = 30
            std_sig = prescription.sig_directions
    ...
    def batch_process(self, prescriptions: List[PrescriptionInput]) -> List[ExtractedData]:
        ...
        for prescription in prescriptions:
            try:
                ...
                logger.error(f"Failed to process prescription {prescription.drug_name}: {e}")
                # Create error result
                error_result = ExtractedData(
                    original_drug_name=prescription.drug_name,
                    matched_drug_name=prescription.drug_name,
                    medication_type=MedicationType.UNKNOWN,
                    corrected_quantity=float(prescription.quantity) if str(prescription.quantity).replace('.', '').isdigit() else 0,
...
def main():
    ...
    test_prescriptions = [
        ...
        PrescriptionInput("Humalog", "5", "take 15 units before meals three times daily"),
        PrescriptionInput("Timolol 0.5%", "5", "1 drop in each eye twice daily"),
        PrescriptionInput("Hydrocortisone 1%", "30", "apply to affected area twice daily"),
